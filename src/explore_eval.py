...
def view(args, results, tg_info):
    ...
    for point_px in results["info"]["points"]:
        cv2.circle(rgb, tuple(point_px), 5, (255, 0, 0), -1)
    
    cv2.imwrite(f"{args.eval_dir}/miss_{name}.png", rgb)
