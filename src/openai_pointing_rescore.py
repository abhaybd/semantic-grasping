...
def get_args():
    ...
    submit_parser.add_argument("pointing_results_path")
    submit_parser.add_argument("-k", "--top-k", type=int, default=2)
...
def init_submit_proc(width: int, height: int, tg_dir: str, pointing_results_path: str):
    globals()["renderer"] = SceneRenderer(width, height, mesh=False)
    globals()["tg_info"] = TaskGraspInfo(tg_dir)
    with open(pointing_results_path, "rb") as f:
        globals()["pointing_results"] = pickle.load(f)

def submit_proc(data_dir: str, scene_name: str, k: int):
    scene = Scene(data_dir, scene_name, 0.0)
    tg_info: TaskGraspInfo = globals()["tg_info"]
    if tg_info.valid_obj(scene.obj_name):
        return generate_query(tg_info, scene, k, globals()["renderer"], globals()["pointing_results"][scene_name])
    else:
        return None
...
def submit_job(args, tg_info: TaskGraspInfo, client: OpenAI):
    if not os.path.isfile(os.path.join(args.out_dir, "batchinput_1.jsonl")):
        ...
        with mp.Pool(args.render_batch_size, initializer=init_submit_proc, initargs=(width, height, args.taskgrasp_dir, args.pointing_results_path)) as pool:
            futures = []
            queue = mp.Queue()
            for scene_name in os.listdir(args.data_dir):
                futures.append(pool.apply_async(submit_proc, (args.data_dir, scene_name, args.top_k), callback=queue.put, error_callback=queue.put))
    ...
    for line in results_lines:
        ...
        try:
            ...
            if m is not None:
                ...
                continue
        if "best_grasp_id" not in response:
            print(f"best_grasp_id not in response for scene {scene}")
            continue
...
def generate_query(tg_info: TaskGraspInfo, scene: Scene, k: int, renderer: SceneRenderer, pointing_result: dict):
    task = tg_info.get_task(scene.obj_name)

    messages = [
        {
            "role": "developer",
            "content": f"You are a robot task with choosing the best grasp for the task \"{task}\", which are represented as grasps drawn onto an image. These grasps will be provided to you as a list of images, and you must choose the best one by replying with the ID of the best grasp. If all grasps are equally unsuitable, default to the first one."
        }
    ]
    ...
    pointed_grasp_ids = sorted(set(pointed_grasp_ids))  # dedupe

    for grasp_id in pointed_grasp_ids:
        img = renderer.render([scene.grasps[grasp_id]], [[255, 0, 0]])
        buf = io.BytesIO()
